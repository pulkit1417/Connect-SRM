...
const ProjectForm = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!title || !description || !gitRepo) {
      toast.error("All fields are required!");
      return;
    }
    setIsLoading(true);
    try {
      await onSubmit({ title, description, gitRepo });
      onClose();
      setTitle("");
      setDescription("");
      setGitRepo("");
        ...
        {event.teamDetails && (
          <div className="mb-4 p-3 bg-gray-100 rounded-lg">
            <p className="text-indigo-600 flex items-center font-semibold">
              ...
              Team: {event.teamDetails.teamName}
            </p>
            <p className="text-gray-600 mt-1 pl-6">
              Lead: {event.teamDetails.teamLeadName}
            </p>
          </div>
        )}
    ...
    try {
      ...
      if (socials.leetcode !== initialData.leetcode) {
        ...
        try {
          ...
          alert(
            "LeetCode username submitted for verification. It will be displayed after verification."
          );
        } catch (leetcodeError) {
      ...
      alert(
        error.message ||
          "An error occurred while updating profiles. Please try again."
      );
    } finally {
      setVerifying(false);
    }
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {Object.entries(socialIcons).map(([key, Icon]) => (
                <motion.div
                  ...
                  transition={{
                    delay: Object.keys(socialIcons).indexOf(key) * 0.1,
                  }}
                  className="relative"
                >
                  <div className="relative bg-gray-100 rounded-lg overflow-hidden group">
                    ...
                    <input
                      ...
                      }${key === "leetcode" ? " (Requires Verification)" : ""}`}
                    />
                    {key === "leetcode" &&
                      socials.leetcodeStatus === "pending" && (
...
const Profile = () => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        const eventList = await Promise.all(
          eventSnapshots.docs.map(async (eventDoc) => {
            const eventData = eventDoc.data();
            const teamsQuery = query(
              collection(db, "events", eventDoc.id, "teams")
            );
            ...
            const userTeam = teamsSnapshot.docs.find((teamDoc) => {
              ...
              ].includes(decodedEmail);
            });
            return {
              id: eventDoc.id,
              ...eventData,
              teamDetails: userTeam
                ? { ...userTeam.data(), teamId: userTeam.id }
                : null,
            };
          })
        );
  ...
  const isLeetCodeVerified = (user) => {
    return (
      user.leetcode &&
      (user.leetcodeStatus === "verified" || user.leetcodeStatus === "accepted")
    );
  };
  ...
  const refreshCredits = async () => {
    ...
    try {
      const response = await fetch(
        `https://mission-api-eight.vercel.app/${user.leetcode}/solved/`
      );
  ...
  const handleAddProject = async (projectData) => {
    try {
      ...
      const newProjectData = {
        ...projectData,
        gitUser: user.gitHub, // Use the user's GitHub username
        expoToken: user.expoToken,
        name: user.name,
        timestamp: `${timestamp.getDate()}/${
          timestamp.getMonth() + 1
        }/${timestamp.getFullYear()} ${timestamp.toTimeString().split(" ")[0]}`,
        author: decodedEmail,
        sort: serverTimestamp(),
      ...
      await addDoc(projectRequestsRef, {
        ...newProjectData,
        dId: newProjectRef.id,
      });
          ...
          {projects.map((project) => (
            ...
            >
              ...
              <div className="flex justify-between items-center">
                <a
                  href={`https://github.com/${user.gitHub}/${project.gitRepo}`}
                  target="_blank"
                  rel="noopener noreferrer"
          ...
          <div className="mb-6">
            ...
            <p className="text-gray-600 mb-6 max-w-md mx-auto">
              {user.gitHub 
                ...
                : "To showcase your projects, you'll need to connect your GitHub account. This helps us verify your contributions and display your work."}
            </p>
          </div>
          {!user.gitHub ? (
            <button
              onClick={handleOpenPopup}
              className="inline-flex items-center px-6 py-3 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 transition-all duration-300 shadow-md hover:shadow-lg"
            >
